...
The CA common name was explicitly set to `MedNet-RootCA` rather than accepting the auto-generated default, to produce a clean, environment-appropriate name that appears consistently across all issued certificates.

Upon successful configuration, Windows automatically issued a domain controller certificate to `dc01.mednet.lab`, enabling LDAPS on port 636.
...
|---|---|
| `TLS_REQCERT never` set on Debian LDAP client | Replace with `TLS_REQCERT demand` and a properly chained CA cert |
...
| DC referenced by IP in osTicket | Configure DNS on the osTicket VM to use the DC as resolver, then reference by FQDN |
