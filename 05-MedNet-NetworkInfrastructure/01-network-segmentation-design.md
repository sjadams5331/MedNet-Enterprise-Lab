...
- Constrain the ITSM platform (not domain-joined, authenticates over LDAPS only) to the minimum server access it actually needs
...
All five VLANs are now live and hold their intended hosts. The original `192.168.56.0/24` VirtualBox Host-only network (`vboxnet0`) that every host previously shared is fully retired; no VM keeps an adapter on it post-migration. pfSense's WAN uplink is a separate, unrelated network, detailed below.

![pfSense Interfaces Assignments page showing all six interfaces mapped and up](../screenshots/01-network-segmentation-design_01.png)
...
- **WAN placement.** pfSense's WAN interface is NAT-attached rather than host-only, receiving a dynamic address (`10.0.2.0/24`) from VirtualBox's built-in NAT DHCP for outbound internet access (updates, package repos, CRL/OCSP checks, etc.). This was a deliberate change from an earlier plan that assumed WAN would ride the original `192.168.56.0/24` Host-only segment; NAT was chosen instead so the lab's internet path is fully decoupled from the internal VLANs it routes between.
